...
const pollCheckoutIntent = async (
  ...
  while (Date.now() - start < timeout) {
    ...
    const traceId = res.headers.get('rye-trace-id');
    if (traceId && onTraceId) onTraceId(traceId);
...
function CheckoutForm({ cost, checkoutIntentId, onBack, onSuccess, onTraceId }: { cost: { currencyCode: string; amountSubunits: number }, checkoutIntentId: string, onBack: () => void, onSuccess: (checkoutIntent: unknown) => void, onTraceId: (traceId: string) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const checkoutIntentObject = await pollCheckoutIntent(data.checkoutIntent.id, ["completed", "failed"], 120000, 5000, onTraceId);
...
// RyeTraceIdPanel: displays all rye-trace-id values
function RyeTraceIdPanel({ traceIds }: { traceIds: string[] }) {
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-4 w-full max-w-md">
      <div className="font-bold mb-2 text-sm">Rye Trace IDs</div>
      {traceIds.length === 0 ? (
        <div className="text-xs text-gray-400">No trace IDs yet.</div>
      ) : (
...
export default function Home() {
  ...
  const traceIdSet = useRef<Set<string>>(new Set());
  ...
  const handleGetCost = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const traceId = res.headers.get('rye-trace-id');
      if (traceId && !traceIdSet.current.has(traceId)) {
        traceIdSet.current.add(traceId);
        setTraceIds(ids => [...ids, traceId]);
      }
      const data = await res.json();
      ...
      const pollData = await pollCheckoutIntent(
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row items-start justify-center bg-gradient-to-br from-white to-gray-100 p-4 gap-8">
      ...
      <div className="flex-1 flex flex-col items-center">
        <RyeTraceIdPanel traceIds={traceIds} />
      </div>
    </div>
  );
}
